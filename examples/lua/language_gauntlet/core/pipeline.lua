...
local Storage = require("domain.storage")
local M = {}
...
function M.orchestrate(envelope, ...)
    ...
    -- Preserve the original aggregate beside the coroutine-derived fold. The
    -- merge is a real runtime value operation and makes the branch join itself
    -- part of the compiler contract even when the iterator yields no tokens.
    joined = joined .. cmd
