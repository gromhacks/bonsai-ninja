...
public enum FlowPipeline {
    ...
    public static func orchestrateAsync(_ envelope: Envelope) async -> String {
        await Task.yield()
        return orchestrate(envelope)
    }

    public static func orchestrate(_ envelope: Envelope) -> String {
        ...
        // Cross a nested source file, receiver-held state, and a trailing
        // closure before the repository hierarchy carries the value onward.
        return StorageGateway.persist(valid)
    }
}
