...
import { execute } from "../infrastructure/executor";
...
export async function persist<T extends { cmd: string }>(data: T): Promise<unknown> {
  ...
  // Exact receiver aliasing plus a getter/setter round trip exercises stored
  // generic object state without changing the value that reaches the sink.
  const alias: AuditedRepository<T> = repo;
  alias.cmd = alias.cmd;
  return alias.run();
}
