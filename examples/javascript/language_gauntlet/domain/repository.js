...
const { execute } = require("../infrastructure/executor");
...
async function persist(data) {
  ...
  // Object alias + getter/setter round trip. Both names designate the exact
  // same receiver, so the stored command must remain attached to it.
  const alias = repo;
  alias.cmd = alias.cmd;
  return alias.run();
}
