//! Focused regressions for compiler-backed security-flow capabilities.
//!
...
fn bin_path() -> PathBuf {
    ...
    assert!(
        release.exists(),
        "bonsai-ninja test binary is required for CLI security regressions"
    );
    release
}
...
fn temp_workspace(tag: &str) -> PathBuf {
    ...
    for attempt in 0..100 {
        let path = base.join(format!(
            "bonsai-security-flow-{tag}-{}-{nanos}-{attempt}",
            std::process::id()
        ));
...
fn run_taint_json_with_flags(ws: &Path, source: &str, sink: &str, flags: &[&str]) -> Vec<Value> {
    let bin = bin_path();
    let cache = temp_workspace("taint-cache");
...
fn run_sinks_json(ws: &Path, rule_regex: &str) -> Vec<Value> {
    let bin = bin_path();
    let cache = temp_workspace("sinks-cache");
...
fn run_sources_json(ws: &Path, rule_regex: &str) -> Vec<Value> {
    let bin = bin_path();
    let cache = temp_workspace("sources-cache");
    let out = Command::new(bin)
        .args(["--no-cache", "--no-progress", "security"])
        .arg(ws)
        .args([
...
#[test]
fn javascript_literal_set_membership_constrains_only_the_exact_selected_value() {
    let vulnerable = temp_workspace("javascript-set-selection-vulnerable");
    write_file(
        &vulnerable,
        "package.json",
        r#"{"dependencies":{"express":"latest","mysql":"latest"}}"#,
    );
    write_file(
        &vulnerable,
        "src/users.js",
...
function users(req, db) {
  return db.query("SELECT * FROM users WHERE role = '" + req.params.role + "'");
}
...
"#,
    ...
    let rows = run_taint_json(
        &vulnerable,
        r"^javascript\.source\.express_req_params$",
        r"^javascript\.sqli\.method_query_concat$",
    );
    assert_has_finding(
        &rows,
        &[
            "javascript.source.express_req_params",
            "javascript.sqli.method_query_concat",
        ],
    );

    let safe = temp_workspace("javascript-set-selection-safe");
    write_file(
        &safe,
        "package.json",
        r#"{"dependencies":{"express":"latest","mysql":"latest"}}"#,
...
function users(req, db) {
  const requested = req.params.role;
  const role = ROLES.has(requested) ? requested : "viewer";
...
#[test]
fn go_nested_request_body_read_reaches_cross_file_gob_decoder_through_cli_pipeline() {
    let ws = temp_workspace("go-nested-request-body-gob");
    write_file(
        &ws,
        "go.mod",
...
"#,
    ...
    let rows = run_taint_json(
        &ws,
        "^go\\.echo\\.context_request_body$",
        "^go\\.deser\\.gob_newdecoder$",
    );
    assert_has_finding(
        &rows,
        &[
...
#[test]
fn ruby_yielded_inline_callback_retains_exact_lexical_capture_in_persisted_idg() {
    let ws = temp_workspace("ruby-yield-capture-path");
    write_file(
        &ws,
        "app/controllers/assets_controller.rb",
        r#"class AssetsController < ApplicationController
  def show
    name = params[:name].to_s
    data = AssetStore.with_root { |root| AssetStore.read(root, name) }
    send_data data
  end
end
"#,
    ...
    write_file(
        &ws,
        "app/models/asset_store.rb",
        r#"class AssetStore
  BASE = "/var/data/assets".freeze
...
"#,
    ...
    write_file(
        &ws,
        "app/models/banner_store.rb",
        r#"class BannerStore
  def self.read
    File.binread("/var/data/assets/banner.txt")
  end
end
"#,
    );
    ...
    assert_rows_do_not_contain(&rows, &["app/models/banner_store.rb"]);

    let clean = temp_workspace("ruby-yield-capture-clean");
    write_file(
        &clean,
        "app/controllers/assets_controller.rb",
        r#"class AssetsController < ApplicationController
  def show
    params[:name].to_s
    AssetStore.with_root { |root| AssetStore.read(root, "banner.txt") }
  end
end
"#,
    );
...
"#,
    ...
    assert_no_finding(&run_taint_json(
        &clean,
        "^ruby\\.source\\.params_read$",
        "^ruby\\.path\\.file_binread$",
    ));
...
#[test]
fn javascript_inline_foreach_callback_carries_receiver_and_lexical_write_to_dom_sink() {
    let vulnerable = temp_workspace("javascript-foreach-capture-dom");
    write_file(
        &vulnerable,
        "src/public/profile.js",
        r#"(function () {
  var hash = window.location.hash || "";
  var displayParam = "";
  hash.replace(/^#?/, "").split("&").forEach(function (part) {
    var kv = part.split("=");
    if (kv[0] === "display") displayParam = decodeURIComponent(kv[1] || "");
  });
  var host = document.getElementById("display-name");
  if (host) host.innerHTML = "<strong>" + displayParam + "</strong>";
})();
"#,
    );
    ...
    let repaired = temp_workspace("javascript-foreach-capture-dom-repaired");
    write_file(
        &repaired,
        "src/public/profile.js",
        r#"(function () {
...
#[test]
fn javascript_inline_foreach_capture_reaches_document_write_and_clean_overwrite_kills_it() {
    let vulnerable = temp_workspace("javascript-foreach-capture-document-write");
    write_file(
        &vulnerable,
  ...
  query.split("&").forEach(function (part) {
    var pair = part.split("=");
    if (pair[0] === "q") term = decodeURIComponent(pair[1] || "");
  });
  document.write("<h2>Results for: " + term + "</h2>");
}
"#,
    );
    let rows = run_taint_json(
        &vulnerable,
        "^javascript\\.source\\.window_location_search$",
        "^javascript\\.xss\\.document_write$",
    );
    assert_has_finding(
        &rows,
        &[
    ...
    let repaired = temp_workspace("javascript-foreach-capture-document-write-repaired");
    write_file(
        &repaired,
        "src/public/search.js",
        r#"function render() {
...
#[test]
fn typescript_koa_location_header_is_open_redirect_without_reclassifying_other_headers() {
    let vulnerable = temp_workspace("typescript-koa-location-open-redirect");
    write_file(
        &vulnerable,
        "src/app.ts",
...
async function route(ctx: any) {
  const target = String(ctx.headers["referer"] || "/");
  ctx.status = 302;
...
"#,
    ...
    let rows = run_taint_json(
        &vulnerable,
        "^typescript\\.source\\.koa_ctx_headers$",
        "^typescript\\.open_redirect\\.koa_location_header$",
    );
    assert_has_finding(
        &rows,
        &[
    ...
    assert_no_finding(&run_taint_json(
        &vulnerable,
        "^typescript\\.source\\.koa_ctx_headers$",
        "^typescript\\.header\\.koa_set$",
    ));

    let literal = temp_workspace("typescript-koa-location-literal");
    write_file(
        &literal,
        "src/app.ts",
        r#"import Koa from "koa";
...
async function route(ctx: any) {
  const ignored = String(ctx.headers["referer"] || "/");
  ctx.set("Location", "/account");
}
"#,
    );
    assert_no_finding(&run_taint_json(
    ...
    let ordinary_header = temp_workspace("typescript-koa-ordinary-header");
    write_file(
        &ordinary_header,
        "src/app.ts",
        r#"import Koa from "koa";
...
async function route(ctx: any) {
  const trace = String(ctx.headers["x-trace"] || "");
  ctx.set("X-Trace", trace);
}
"#,
    );
    assert_no_finding(&run_taint_json(
...
#[test]
fn javascript_compound_request_alias_reaches_cross_file_sink_and_field_overwrite_cuts_it() {
    let vulnerable = temp_workspace("javascript-compound-request-alias");
    write_file(
        &vulnerable,
        "package.json",
        r#"{"dependencies":{"express":"latest","safe-eval":"latest"}}"#,
    );
    write_file(
        &vulnerable,
        "src/runner.js",
        r#"const safeEval = require("safe-eval");
...
"#,
    ...
    write_file(
        &vulnerable,
        "src/routes.js",
        r#"const { evaluate } = require("./runner");

function route(req) {
  const body = req.body || {};
  const source = String(body.expression || "");
...
"#,
    ...
    let clean = temp_workspace("javascript-compound-request-alias-clean");
    write_file(
        &clean,
        "package.json",
        r#"{"dependencies":{"express":"latest","safe-eval":"latest"}}"#,
...
#[test]
fn typescript_compound_request_alias_reaches_cross_file_sink_and_field_overwrite_cuts_it() {
    let vulnerable = temp_workspace("typescript-compound-request-alias");
    write_file(
        &vulnerable,
        "package.json",
...
export function route(req: any): string {
...
"#,
    ...
    let clean = temp_workspace("typescript-compound-request-alias-clean");
    write_file(&clean, "package.json", r#"{"dependencies":{"express":"latest"}}"#);
    write_file(
        &clean,
        "src/run.ts",
        r#"import { execSync } from "node:child_process";
...
#[test]
fn objc_forwarded_dispatch_callback_retains_lexical_capture_in_scoped_idg() {
    let vulnerable = temp_workspace("objc-forwarded-dispatch-capture");
    write_file(
        &vulnerable,
        "Sources/FetchHandler.m",
        r#"#import <Foundation/Foundation.h>

@implementation FetchHandler
- (GCDWebServerResponse *)handleFetch:(GCDWebServerRequest *)request {
...
"#,
    ...
    write_file(
        &vulnerable,
        "Sources/FetchQueue.m",
        r#"#import <Foundation/Foundation.h>

@implementation FetchQueue
...
"#,
    ...
    write_file(
        &vulnerable,
        "Sources/FetchClient.m",
        r#"#import <Foundation/Foundation.h>

@implementation FetchClient
...
"#,
    ...
    let rows = run_taint_json(
        ...
        "^objc\\.ssrf\\.nsdata_contents_of_url$",
    );
    assert_has_finding(
        &rows,
        &[
    ...
    let clean = temp_workspace("objc-forwarded-dispatch-clean");
    write_file(
        &clean,
        "Sources/FetchHandler.m",
        r#"#import <Foundation/Foundation.h>
...
- (GCDWebServerResponse *)handleFetch:(GCDWebServerRequest *)request {
    request.query[@"url"];
    [FetchQueue submit:^{
        [FetchClient get:@"https://api.partner.example/status"];
    }];
    return nil;
}
@end
...
fn lua_luasql_execute_requires_matching_package_evidence() {
    ...
    write_file(
        &ws,
        "luasql.lua",
        r#"local luasql = require("luasql.mysql")

function lookup(name)
  local environment = luasql.mysql()
  local conn = environment:connect("app", "user", "password")
...
#[test]
fn typescript_graphql_runtime_forwards_request_variables_into_static_resolver_map() {
    let ws = temp_workspace("ts-graphql-runtime-map");
    write_file(
        &ws,
        "src/routes.ts",
...
const root = {
  products: ({ filter }: { filter: string }) => search(filter),
};
const router = Router();
...
export function search(filter: string) {
  return pool.query("SELECT * FROM products WHERE name ILIKE '%" + filter + "%'");
}
"#,
    );
...
#[test]
fn typescript_graphql_runtime_does_not_invoke_a_mutable_callback_map_by_assumption() {
    let ws = temp_workspace("ts-graphql-runtime-mutable-map");
    write_file(
        &ws,
        "app.ts",
        r#"import { graphql } from "graphql";
import { Pool } from "pg";
const pool = new Pool();
let root = { products: ({ filter }: { filter: string }) => pool.query("SELECT * FROM p WHERE n='" + filter + "'") };
...
#[test]
fn javascript_graphql_runtime_forwards_request_variables_into_static_resolver_map() {
    let ws = temp_workspace("js-graphql-runtime-map");
    write_file(
        &ws,
        "src/routes.js",
        r#"const express = require("express");
...
const { search } = require("./products");
const root = { products: ({ filter }) => search(filter) };
const router = express.Router();
router.post("/query", async (req, res) => {
  const variables = req.body;
  const query = "query Products($filter: String!) { products(filter: $filter) }";
  res.json(await graphql({ schema: {}, source: query, rootValue: root, variableValues: variables }));
});
"#,
    );
...
const pool = new Pool();
exports.search = function search(filter) {
  return pool.query("SELECT * FROM products WHERE name ILIKE '%" + filter + "%'");
};
"#,
    );
...
#[test]
fn python_graphql_args_reach_factory_typed_connection_execute_sql_sink() {
...
#[test]
fn python_sql_wrapper_reports_the_exact_caller_composition_and_keeps_bound_sql_quiet() {
    let ws = temp_workspace("python-sql-wrapper-composition-location");
    write_file(
        &ws,
        "app/api.py",
        r#"from flask import request
from products import unsafe_search, safe_search

def unsafe_endpoint():
    return unsafe_search(request.args.get("q", ""))

def safe_endpoint():
    return safe_search(request.args.get("q", ""))
"#,
    );
    write_file(
        &ws,
...
def unsafe_search(term):
    sql = ("SELECT id FROM products "
           "WHERE name = '" + term + "'")
    return engine.execute(sql)

def safe_search(term):
    return engine.execute("SELECT id FROM products WHERE name = ?", (term,))
"#,
    );
    write_file(
        &ws,
...
"#,
    ...
    let rows = run_taint_json(
        &ws,
        r"^python\.flask\.request_args_get$",
        r"^python\.sqli\.cursor_execute$",
    );
    assert_eq!(rows.len(), 1, "only the concatenated query is unsafe: {rows:#?}");
    let sink = &rows[0]["sink"];
    assert!(
        sink["file"]
            .as_str()
            .is_some_and(|file| file.ends_with("app/products.py")),
        "the finding must point at the caller-side composition, not the reusable DB wrapper: {rows:#?}"
    );
    assert_eq!(sink["enclosing_fn"], "unsafe_search");
    assert_eq!(sink["line"], 5);
}
...
#[test]
fn swift_and_objc_process_executable_writes_ignore_argv_and_local_variables() {
...
"#,
    ...
    let rows = run_sinks_json(&swift, "^swift\\.cmdi\\.(launchpath_write|executableurl_write)$");
    assert_has_row(
        &rows,
        &[
...
"#,
    ...
    assert_has_row(&rows, &["objc.cmdi.nstask_setters", "task.launchPath"]);
    assert_rows_do_not_contain(
        &rows,
        &[
            "\"text\": \"launchPath\"",
...
class App {
  ...
  void configure(XmlReaderSettings settings, XmlReaderSettings reader) {
...
func local(input: Bool, predicateValue: NSPredicate, headers: [String: String]) {
  let allHTTPHeaderFields = headers
  let predicate = predicateValue
  let shouldResolveExternalEntities = input
}

func configure(input: Data, predicateValue: NSPredicate, headers: [String: String]) {
...
void configure(NSXMLParser *parser) {
...
fn rust_async_graphql_context_args_reach_schema_execute_without_helper_overtaint() {
    ...
    write_file(
        &ws,
        "src/lib.rs",
        r#"use async_graphql::{Context, Schema as ExternalSchema};

fn resolve(ctx: &Context, schema: &ExternalSchema) {
    let query = ctx.args();
...
#[test]
fn php_constructor_injected_shared_state_crosses_an_exact_method_call() {
    let ws = temp_workspace("php-injected-shared-state");
    write_file(
        &ws,
        "src/RequestContext.php",
        r#"<?php
namespace App;
final class RequestContext { public string $assetName = ''; }
"#,
    );
    write_file(
        &ws,
...
final class AssetStore {
  ...
  public function readCurrent(): string {
    $path = '/var/data/assets/' . $this->ctx->assetName;
...
"#,
    ...
    write_file(
        &ws,
        "src/AssetController.php",
        r#"<?php
namespace App;
use Psr\Http\Message\ServerRequestInterface as Request;
final class AssetController {
  public function __construct(private AssetStore $store, private RequestContext $ctx) {}
  public function show(Request $request): string {
    $this->ctx->assetName = $request->getQueryParams()['name'] ?? '';
...
"#,
    ...
    let rows = run_taint_json(
        &ws,
        r"^php\.source\.slim_request_get_params$",
        r"^php\.path\.file_get_contents$",
    );
    assert_has_finding(
        &rows,
        &[
    ...
    let clean = temp_workspace("php-distinct-injected-state");
    write_file(
        &clean,
        "src/Contexts.php",
        r#"<?php
namespace App;
final class ControllerContext { public string $assetName = ''; }
final class StoreContext { public string $assetName = 'banner.txt'; }
"#,
    );
    write_file(
        &clean,
...
namespace App;
final class AssetStore {
  public function __construct(private StoreContext $ctx) {}
  public function readCurrent(): string {
    $path = '/var/data/assets/' . $this->ctx->assetName;
    return (string) file_get_contents($path);
  }
...
use Psr\Http\Message\ServerRequestInterface as Request;
final class AssetController {
  public function __construct(private AssetStore $store, private ControllerContext $ctx) {}
  public function show(Request $request): string {
    $this->ctx->assetName = $request->getQueryParams()['name'] ?? '';
    return $this->store->readCurrent();
  }
