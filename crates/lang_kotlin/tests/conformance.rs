...
#[test]
fn singleton_object_declaration_synthesizes_class_decl() {
    // tree-sitter-kotlin parses `object Foo { ... }` as an
    // `infix_expression` with `object` in the operator field; the adapter
    // synthesizes a class decl so the contained methods have a parent to
    // dispatch into. Regression test for the synthesis being gated on the
    // wrong node kind (`object_literal`), which left it permanently dead.
...
"#,
    ...
    assert!(
        decls
            .iter()
            .any(|decl| decl.name == "Registry" && decl.kind == DeclKind::Class),
        "object Registry should synthesize a Class decl; got {:?}",
        decls.iter().map(|d| (&d.name, &d.kind)).collect::<Vec<_>>()
    );
}
