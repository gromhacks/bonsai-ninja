...
// ===========================================================================
// Solidity — if/for/while/do-while/try/catch/revert/return/assign.
// ===========================================================================

#[test]
fn solidity_hell_chain() {
    // Varies: plain file, `import ... as X`, `import {A}`, `import {A as B}`,
    // and plain-plus-specific imports along the chain.
    let adapter: bonsai_lang_api::AdapterArc = Arc::new(bonsai_lang_solidity::SolidityAdapter::new());
    let files = [
        (
            "/w/m00_entry.sol",
            // plain import
            "pragma solidity ^0.8.0;\nimport \"./m01_if.sol\";\ncontract M00Entry { function entry() public returns (string memory) { return M01If(address(0)).stepIf(\"tok\"); } }\n",
        ),
        (
            "/w/m01_if.sol",
            // import ... as Alias
            "pragma solidity ^0.8.0;\nimport \"./m02_for.sol\" as ForMod;\ncontract M01If { function stepIf(string memory x) public returns (string memory) { if (bytes(x).length > 0) { return ForMod.M02For(address(0)).stepFor(x); } return \"\"; } }\n",
        ),
        (
            "/w/m02_for.sol",
            // import {Symbol}
            "pragma solidity ^0.8.0;\nimport {M03While} from \"./m03_while.sol\";\ncontract M02For { function stepFor(string memory x) public returns (string memory) { for (uint i=0; i<1; i++) { return M03While(address(0)).stepWhile(x); } return \"\"; } }\n",
        ),
        (
            "/w/m03_while.sol",
            // import {Symbol as Alias}
            "pragma solidity ^0.8.0;\nimport {M04Do as Doer} from \"./m04_do.sol\";\ncontract M03While { function stepWhile(string memory x) public returns (string memory) { uint i=0; while (i<1) { i++; return Doer(address(0)).stepDo(x); } return \"\"; } }\n",
        ),
        (
            "/w/m04_do.sol",
            // plain
            "pragma solidity ^0.8.0;\nimport \"./m05_try.sol\";\ncontract M04Do { function stepDo(string memory x) public returns (string memory) { uint i=0; do { i++; return M05Try(address(0)).stepTry(x); } while (i<1); } }\n",
        ),
        (
            "/w/m05_try.sol",
            // import ... as Alias
            "pragma solidity ^0.8.0;\nimport \"./m06_catch.sol\" as CatchMod;\ncontract M05Try { function stepTry(string memory x) public returns (string memory) { try CatchMod.M06Catch(address(0)).stepCatch(x) returns (string memory r) { return r; } catch { return \"\"; } } }\n",
        ),
        (
            "/w/m06_catch.sol",
            // import {Symbol}
            "pragma solidity ^0.8.0;\nimport {M07Assign} from \"./m07_assign.sol\";\ncontract M06Catch { function stepCatch(string memory x) public returns (string memory) { try M07Assign(address(0)).stepAssign(x) returns (string memory r) { return r; } catch { revert(\"boom\"); } } }\n",
        ),
        (
            "/w/m07_assign.sol",
            // import {Symbol as Alias}
            "pragma solidity ^0.8.0;\nimport {M08Return as Ret} from \"./m08_return.sol\";\ncontract M07Assign { function stepAssign(string memory x) public returns (string memory) { string memory y = Ret(address(0)).stepReturn(x); return y; } }\n",
        ),
        (
            "/w/m08_return.sol",
            // plain
            "pragma solidity ^0.8.0;\nimport \"./m09_revert.sol\";\ncontract M08Return { function stepReturn(string memory x) public returns (string memory) { return M09Revert(address(0)).stepRevert(x); } }\n",
        ),
        (
            "/w/m09_revert.sol",
            // import ... as Alias
            "pragma solidity ^0.8.0;\nimport \"./m10_sink.sol\" as Sk;\ncontract M09Revert { function stepRevert(string memory x) public returns (string memory) { string memory r = Sk.M10Sink(address(0)).sink(x); revert(r); } }\n",
        ),
        (
            "/w/m10_sink.sol",
            "pragma solidity ^0.8.0;\ncontract M10Sink { event Sink(string); function sink(string memory x) public returns (string memory) { emit Sink(x); return x; } }\n",
        ),
    ];
    let chain = [
        "entry",
