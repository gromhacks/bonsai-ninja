...
fn symbol_summary(
    ...
    if !unresolved_parameter_spans.is_empty() {
        analysis_incomplete_reasons.push(format!(
            "{} parameter-dispatched call site(s) have no compiler-proven binding",
...
fn unresolved_parameter_dispatch_spans(
    events: &[FlowEvent],
    params: &[String],
    ...
    for_each_flow_event(events, &mut |event| {
        ...
        {
            if call_invokes_parameter(params, name, receiver.as_deref())
