...
never an analysis budget. The workspace's exact-body cache also derives its
byte budget from this effective memory limit. That cache changes how much
adapter-lowered IR is retained between exact lookups, not which IR is produced
or which files are analyzed.
