...
Since schema v7 (the current schema is v12,
`schemas/bonsai-native-export-v12.schema.json`) code locations are serialized
...
into metadata validation without weakening exact source identity. A legacy
in-tree `<workspace>/.bonsai` directory left by older releases is never read;
`cache stats` reports it and `cache clear --legacy`
(`WorkspaceCache::clear_legacy_in_tree`) removes it.
