...
construction. Production security surfaces are IDG-backed: source seeds are
...
corridor before call records or findings are rendered. The older
`interprocedural_taint_to_completion_with_caches` path remains as the
function-summary engine used by diagnostics and compatibility surfaces; it is
not the production source-to-sink scheduler. `reachable.rs` also owns the
...
- `ValueFlowCache` (`value_flow.rs`) — per-`FuncId` seed-free
  ...
  Loaded when present and computed for the requested scope; persisted to
...
8. `taint` prewarms compatibility dataflow/value-flow caches. Production
   security scans build source-specific IDG graphs on demand and reuse the
...
9. `DataFlowCache::prewarm_all` and `ValueFlowCache::prewarm_all`
   ...
   `.bonsai/dataflow.v3.factstore` / `.bonsai/value_flow.v*.factstore`.
