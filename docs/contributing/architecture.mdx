...
`sink-analysis` reuses taint-analysis's endpoint-matching pass, compiles one
sink-relevant compiler/IDG scope, and validates candidate value origins
forward against exact sink nodes. Source-to-sink security proofs are grouped
separately under the same endpoint; source filters cannot erase compiler
lineage. It does not run a backward graph walk per sink. None of these surfaces uses
