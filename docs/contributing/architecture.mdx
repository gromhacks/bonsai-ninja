...
- `DataFlowCache` (`dataflow.rs`) — per-`FuncId` `KindedTokens` +
  ...
  open; persisted to `.bonsai/dataflow.v2.bin`.
- `ValueFlowCache` (`value_flow.rs`) — per-`FuncId` seed-free
  ...
  Prewarmed at open; persisted to `.bonsai/value_flow.v2.bin`.
...
- `TaintGraphIndex` (`taint_index.rs`) — workspace-wide
  ...
  `.bonsai/taint_graph.v1.bin` with version + fingerprint guard.
...
9. `DataFlowCache::prewarm_all` and `ValueFlowCache::prewarm_all`
   ...
   `.bonsai/dataflow.v2.bin` / `.bonsai/value_flow.v2.bin`.
