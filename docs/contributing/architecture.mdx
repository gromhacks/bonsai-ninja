...
9. Dataflow facts are persisted to `.bonsai/dataflow.v2.bin`.

Subsequent runs read the dataflow sidecar instead of repeating the work,
with source-content and dependency-hash checks that invalidate stale facts.
`index --watch` keeps the workspace open, reparses saved file changes in
place, tombstones deleted files, and rewrites the sidecar after affected
entries are rebuilt.
