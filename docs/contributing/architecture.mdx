...
```
 ...
 bonsai_parser  bonsai_cfg  bonsai_index  bonsai_lang_* adapters
                       ...
                       bonsai_lang_api
                              ▲
                 bonsai_diagnostics  bonsai_vfs
                              ▲
                        bonsai_common
                              ▲
          bonsai_hash  bonsai_factstore  bonsai_rulepack
```
...
portable `scripts/audit-layering.sh` gate checks normal and build dependencies,
ignores development-only edges, and rejects upward production edges.
...
outer decoded segment for same-segment receiver-state projection and the
numeric address pages for cross-segment projection; it must not reopen and
MessagePack-decode a segment once per projected edge. It stores matched call
and return boundaries in sorted sparse source rows and does not retain a
second workspace-sized endpoint relation.
...
`path` builds an exact compressed corridor from the cached resolved callgraph
plus warmed IDG cross-call edges when an IDG sidecar has been hydrated.
Forward/reverse fixed points retain every exact/narrowed node and edge that is
both reachable from a selected source and able to reach a selected target.
The command never enumerates concrete path combinations, so diamonds and
cycles remain linear in graph size. Edge precision and resolver provenance
...
callable declaration (for example an external API), `path` retains the
...
primitives for `inspect`. Structural graph-flow output owns one source-backed
evidence unit per matching callable; it does not enumerate recursive caller
paths. `symbol-summary` exposes the corresponding declaration packet with
direct resolved neighbors and explicit unresolved calls, including
...
`comments`, `search`, `symbol-summary`, `path`, `slice`, `read-file`, `tree`). Most commands
