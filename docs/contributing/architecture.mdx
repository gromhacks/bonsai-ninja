...
detected from the host and Linux cgroups, optionally lowered by
...
safety margin; the conservative worker profile serializes exact units at a
3 GiB budget, while larger budgets allow small units to share only the
remaining parallelism. IDG transfer obtains the same exact VFS byte length
through the compiler semantics provider after linkage/resolver state is
resident. Unsupported RSS probes fall back to a conservative reserve. This is
never an analysis budget.
...
IDG as ordered worker processes. The process boundary returns Tree-sitter and
...
On first query, forward and backward numeric CSR adjacency is counted and
filled together in two passes over canonical IDG edges. It does not stage a
second workspace-sized endpoint vector, perform name search, or change the
edge set. Precision-specific adjacency remains lazy so unused query modes do
not duplicate the graph in memory or on disk.
...
- `TaintGraphIndex` (`taint_index.rs`) — workspace-wide
  ...
  `.bonsai/taint_graph.v*.<mode>.<config>.factstore` with version + fingerprint
...
- `TransitiveCallersIndex` (`transitive_callers.rs`) — cached,
  explicitly depth-scoped caller-frontier queries on the resolved callgraph.
  This is a candidate/scheduling utility, not the production taint closure;
  the semantic IDG fixed point is uncapped.
