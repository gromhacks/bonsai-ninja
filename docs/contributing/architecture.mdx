...
`compiler-objects.v<frontend-ABI>.factstore`, with its version defined by
`COMPILER_OBJECT_CACHE_VERSION` in that module; every
...
Swift computed getters are separate executable declarations. They retain the
ordinary lowered body events and exact call-argument/callback projections, not
only a synthesized return expression. Nested callback bodies keep separate
owners. Changes to synthesized lowering invalidate the compiler-object frontend
ABI and all derived semantic sidecars just like ordinary callable changes.

Python string-composition facts include direct call arguments as well as
assignments and returns. Argument projections join the existing typed argument
directory through exact Tree-sitter range lookup; they do not parse rendered
text or rescan the whole tree per argument. Unsupported value shapes remain
unknown rather than receiving a partial safety proof.
...
Its separate CFG-local parameter projections must also reach a fixed point.
Copying descendant taint reports whether the RHS carries a field, not whether
inserting that field changed a set. Capture the RHS before overwriting the
target so repeated copies remain stable, self-copies retain their facts, and
old target fields do not survive replacement. These local projections are not
a second interprocedural engine or a security finding.
Since schema v7 (the current schema is v14,
`schemas/bonsai-native-export-v14.schema.json`) code locations are serialized
...
into metadata validation without weakening exact source identity. Git change
proofs are usable only when Git can observe the selected compiler inputs.
An explicitly selected workspace ignored by its parent repository uses the
filesystem fallback for manifest validation and live SDK refresh; an empty
Git status there cannot prove that files were unchanged, added, or removed.
Legacy in-tree analysis files are not reused. `cache stats` reports recognized
...
that function per parameter, one per exact storage-read node another summary writes
...
Summary composition uses an immutable scoped IDG boundary directory prepared
before workers start. Its exact argument/formal indexes preserve receiver and
named-argument bindings; callgraph adjacency alone is not a binding proof.
Separate reads of the same storage name remain separate entering values.
Origin witnesses propagate through the compiled summary graph once, so a
dead-end or cyclic predecessor cannot hide another rooted route. Projected
heap steps retain `storage_transfer` evidence and never become synthetic
direct-call records or positional arguments.
