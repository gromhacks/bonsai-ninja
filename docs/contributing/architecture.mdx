...
The object generation exposes two separately integrity-checked header
projections: imports, plus compact syntax targets containing adapter-lowered
calls, assignment aliases, factory assignments, and receiver/type evidence.
Rule planning first tests raw source anchors, then exact import/package
headers, then exact syntax targets. Only surviving files decompress
declarations and flow events. The full body remains the canonical source when
a surviving file needs matching, transfer, or rendered evidence; this is one
compiler IR with staged decoding, not a second parser.
