...
inheritance, import, and stable-symbol headers. Security, inspect-graph, export, and
...
`TAINT TRANSFORM` is a separate evidence role: an operation changes the value's
representation without proving safety for its use. URL encoding, URL building,
and context-free shell quoting do not establish a trusted destination or
executable. Such matches stay out of the sanitizer inventory and cannot alone
change a finding to `sanitized`. See
[Sanitizer credit](security-spec.mdx#sanitizer-credit) for the exact contract.
...
`FLOW`, `SOURCE n · DOWNSTREAM FLOW m`, `SINK n · UPSTREAM FLOW m`, and
`TAINT FLOW` are intentionally different text labels. Source and sink flow
headers retain the numbered parent endpoint and its rule id so a paged report
never separates lineage from the boundary it explains.

| Label | Produced by | Meaning |
|---|---|---|
| `FLOW` | `inspect-graph` (`FLOW N` blocks in the default `trace` view, `GROUP N` blocks with `--view grouped`, `CORRIDOR FLOW` for `--from`/`--to`) | A stable structural evidence unit. For an ordinary `inspect-graph` query this is one matched callable; `--from`/`--to` queries use an exact compressed compiler corridor rather than enumerating every path combination. |
| `SOURCE n · DOWNSTREAM FLOW m` | `security source-analysis` | A path produced by running taint forward from numbered source `n`. It maps where boundary data can travel without requiring a sink. |
| `SINK n · UPSTREAM FLOW m` | `security sink-analysis` | Exact source-independent compiler value lineage into numbered sink `n`, presented sink-first. Security-source reachability is separate evidence. |
