...
`FLOW`, `SOURCE FLOW`, `UPSTREAM FLOW`, and `TAINT FLOW` are intentionally
...
| `UPSTREAM FLOW` | `security sink-analysis` | Exact source-independent compiler value lineage into one matched sink, presented sink-first. Security-source reachability is separate evidence. |
