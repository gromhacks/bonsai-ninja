...
### STATUS: deployment checks are GREEN for all 21 parser adapters
...
- `cargo test --release --workspace --no-fail-fast` is green as of 2026-06-06, including doc tests and the 21-language CLI, inspect, adapter, taint, cache, SARIF, and mega-flow suites.
...
- Security regression suites passed: `rulepack_conformance` 28/28, `security_pipeline_regressions` 16/16, `branch_merge_audit` all 21, `try_catch_audit` all 21, `callback_flow_audit` all 21, `cross_file_chain_audit` all 21, and `sanitizer_fixtures` 6/6.
...
The `examples/<lang>/mega_flow` fixtures are structurally identical POSITIVE flows in all 21 parser adapters (`SOURCE -> handle/main -> orchestrate -> persist -> run -> execute -> SINK`). Detection status (via `security … taint-analysis --inferred-sources`, fresh sidecars) - **all adapters now detect their intended synthetic source-to-sink chain**. For Solidity this is an adapter/flow correctness fixture, not a claim that Solidity belongs to the CWE-78 app/web command-injection taxonomy:
...
- **Bottom line: all 21 mega_flow adapters now detect their intended synthetic source-to-sink flow through their full FN-language construct stack.** For Solidity this is a smart-contract adapter/flow check rather than a CWE-78 app/web command-injection claim. mega-flow 14/14, no_fp_audit 1/1 (covers 136 cases), idg 193+5, rulepack_conformance 28, workspace --lib 59, all per-lang adapter tests green.
...
- **Git bisection results so far (2026-05-29):**
  ...
  - Fails on `d332009` (Detect mega_flow source-to-sink flow across all 21 parser adapters).
