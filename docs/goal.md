...
- Java source-to-sink scheduling verification passed on 2026-06-04 with semantic IDG function reachability to fixpoint. The taint budget no longer caps source eligibility. Per-source taint walks use a source-to-target IDG cut when a semantic sink corridor exists, and source groups with no semantic corridor are skipped instead of falling back to a whole-graph walk. Release-binary scale checks covered 10,000 same-function Java flows in 9.53s, 5,000 two-file flows across 10,000 files in 5.88s, 9,999 entries to one shared sink helper in 10.69s, and 1 real flow plus 1,000 unreachable source-only files in 0.61s with source groups filtered from 1,001 to 1. OWASP Benchmark production taint-analysis default-page verification reported 1,745 total findings in 12.83s and built taint chains in 1.954s.
...
- Security regression suites passed: `rulepack_conformance` 28/28, `security_pipeline_regressions` 15/15, `branch_merge_audit` all 21, and `sanitizer_fixtures` 6/6.
