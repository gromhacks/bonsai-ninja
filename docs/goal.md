...
> behavior. Commands, flags, counts, timings, and readiness headings below are
> historical snapshots and may not match the current binary; do not copy them
> into current workflows without checking `--help`.
...
## Historical deployment-readiness snapshot - 2026-06-19

### STATUS: local release/build checks are green
...
OWASP Benchmark v1.2 direct category-aware scoring reported 54.54 overall,
65.87% TPR, 11.32% FPR,
LDAP 66.67% TPR / 0.00% FPR, XPath 66.67% TPR / 10.00% FPR, and SQLi
63.24% TPR / 2.16% FPR.

The latest security-quality status is tracked in
[Release Readiness](RELEASE_READINESS.md). Current fixed-snapshot work
should distinguish true code-changed fixes from unchanged or
metadata-only validation snapshots before treating an aggregate
fix-validation miss as a sanitizer gap.

## Historical deployment-readiness snapshot - 2026-06-07

### STATUS: deployment checks are GREEN for all 20 parser adapters
...
- `cargo test --release --workspace --no-fail-fast` is green as of 2026-06-06, including doc tests and the 20-language CLI, inspect, adapter, taint, cache, SARIF, and mega-flow suites.
- 20-language SARIF smoke output passed via `cargo test -p bonsai_cli --test per_lang_cli_matrix micro_security_sarif_shape -- --nocapture`.
- 20-language mega-flow CLI coverage passed via `cargo test -p bonsai_cli --test security_commands taint_analysis_run_across_every_mega_flow_lang -- --nocapture`.
- Release CLI command/switch validation passed with `python3 scripts/validate-mega-cli.py --bin ./target/release/bonsai-ninja --skip-realworld`.
...
- Real-world large Java verification passed against the local Elasticsearch checkout with the reported command shape, `security ../elasticsearch taint-analysis --context 512k --no-color --no-progress`. The release run completed in 121.25s with 327 total findings and no `payload exceeds 4GiB` or `save_to_disk failed` warning. Phase timing with `BONSAI_DEBUG=security-phase` showed source matching in 6.776s, sink matching in 5.200s, sanitizer matching in 4.685s, source-reachable callgraph construction in 11.968s, scoped semantic graph construction in 6.939s, source-sink reachability in 0.004s, taint-chain building in 23.262s, flow evidence in 0.012s, and first-page rendering in 1.011s. The source-reachable graph was scoped to 15,557 functions / 3,074 files from 2,018 source functions and 7,954 sink functions, reaching 1,745 sink functions; the IDG scope was further narrowed to 13,876 functions / 2,958 files. Source groups were filtered from 2,018 to 497. Large Java scopes now disable eager field-argument forwarding and broad receiver-method propagation during the scoped IDG build; the exact taint graphs still run over semantic IDG/callgraph corridors instead of whole-project graph walks.
- Source-to-sink scheduling verification passed on 2026-06-04 and was tightened again on 2026-06-06 with semantic IDG function reachability to fixpoint across all 20 parser adapters. The taint budget no longer caps source eligibility. Per-source taint graph builds use a source-to-target IDG cut when concrete seed and sink nodes are available, use a source-reachable resolved-callgraph corridor when the concrete node cut cannot prove completeness, skip source groups with no semantic source-to-sink corridor, and never fall back to a workspace-wide graph. Same-file source batches get a concrete seed-node to sink-node prefilter so source-only calls do not schedule taint work. Cross-file node-cut uncertainty stays scheduled and is decided by the normal corridor-scoped semantic taint graph to avoid false negatives. Release-binary scale checks covered 10,000 same-function Java flows in 9.53s, 5,000 two-file flows across 10,000 files in 5.88s, 9,999 entries to one shared sink helper in 10.69s, and 1 real flow plus 1,000 unreachable source-only files in 0.61s with source groups filtered from 1,001 to 1. OWASP Benchmark production taint-analysis default-page verification reported 1,745 total findings in 12.83s and built taint chains in 1.954s.
- All-language 10,000-file mega-flow scalability verification passed on 2026-06-06 against generated `/tmp` workspaces that were deleted after the run. Each workspace copied only valid `examples/<lang>/mega_flow` source files into isolated module/package directories. Exact content checks passed for every language: expected finding count, finding language, source and sink rule ids, source/sink files, flow ids, and taint-path files all stayed inside the matching copy. Release `index` ranged from 1.77s to 4.33s. Release `security taint-analysis --format json --all --no-color --no-progress` ranged from 4.56s to 40.19s across the 20 languages; Java was 2,500 findings in 40.19s and Swift was 2,500 findings in 38.26s. Full timing details are mirrored in `docs/rule-testing.mdx`.
- Real-world Elasticsearch-sized non-Java corpus verification passed on 2026-06-07 with the then-current release binary and the disposable-clone harness, `python3 scripts/realworld-lang-benchmark.py --bin ./target/release/bonsai-ninja --langs all --command-timeout 1500 --clone-timeout 1200 --continue-on-error`. The harness shallow-cloned each repository under `/tmp`, ran `index`, ran production `security taint-analysis --format json --all --no-color --no-progress` with `BONSAI_DEBUG=security-phase`, validated finding content rather than only JSON structure, recorded phase timing, and deleted every checkout after its audit. Content validation checked stable finding ids, source/sink objects, `analysis_complete`, source/sink/taint-path files under the disposable workspace, existing referenced files, language counts, severity counts, precision counts, and forbidden persistence warnings. All 19 listed non-Java targets completed with `workspace_deleted: true`, no validation problems, no timeouts, no `payload exceeds 4GiB` warning, and no `save_to_disk failed` warning.
...
| TypeScript | VS Code | 10,923 | 22.332s | 37.767s | 3 |
  ...
  The real-world matrix fixed several scale regressions that synthetic fixtures did not cover: TypeScript/JavaScript relative-import matching now uses direct VFS lookup instead of scanning all files per import; C/C++ large literal initializers and translation-unit private scans avoid recursive stack blowups; Scala constructor inheritance has a self-cycle guard for external qualified bases that simplify to a local class name; Swift-style parser stress files with extreme delimiter nesting are skipped at ingest before tree-sitter can recurse deeply; Rust release workers default to a 64MiB Rayon stack; and IDG descendant storage indexing now reads raw Read/Write places instead of resolving every closure node. Taint scheduling remains source-to-sink semantic corridor based: the concrete node cut is used when selective and complete, otherwise a source-origin resolved-callgraph corridor is used. Summary-output functions can climb return-to-caller edges only along source-origin output corridors, which preserves returned-object flows such as Ruby `wrap(data) -> run -> execute` without expanding TypeScript/Scala scopes to unrelated callers. There is no workspace-wide graph fallback.
- Synthetic 10,000-flow Python `mega_flow` scalability verification passed against a generated `/tmp` workspace that was deleted after the run. The test copied the Python `mega_flow` into 80,000 valid source files with unique modules, symbols, routes, and imports. `index` covered all 80,000 files in 22.74s. Source inventory returned exactly 20,000 `python.flask.request_args_get` rows in 36.14s, sink inventory returned exactly 20,000 `python.cmdi.os_system` rows in 35.80s, and `defs --kind function` returned exactly 300,000 rows in 30.40s (flow columns are now opt-in with `--flows`). Cold production taint returned exactly 10,000 verified source-to-sink findings in 124.49s; warm taint returned the same 10,000 rows in 64.15s with byte-identical JSON. The cold IDG build produced 8,300,000 total edges, receiver-method propagation completed in 1.699s, the warm run loaded the saved IDG sidecar in 1.496s, and no `payload exceeds 4GiB` or `save_to_disk failed` warning occurred. Compact benchmark details are in `target/python-megaflow-10000-benchmark.json` when the local target directory is preserved.
- Sharded duplicate-class scale verification passed against a generated `/tmp` Python workspace that was deleted after the run. The workspace contained 10,000 valid source-to-sink flows across 80,100 files, with every flow importing its own `CommandRunner` while all flows reused the same `CommandRunner.execute` class/method names. Exact content checks passed: 10,000 `perform_* -> execute` edges in 43.64s with every caller mapped only to its matching executor, 20,000 Flask `request.args.get` remote sources in 27.50s, 20,000 high-severity `os.system` sinks in 28.35s, 10,000 cold taint findings in 94.32s, and 10,000 warm taint findings in 58.05s. No `payload exceeds 4GiB` or `save_to_disk failed` warning occurred.
- Security regression suites passed: `rulepack_conformance` 28/28, `security_pipeline_regressions` 16/16, `branch_merge_audit` all 20, `try_catch_audit` all 20, `callback_flow_audit` all 20, `cross_file_chain_audit` all 20, and `sanitizer_fixtures` 6/6.
...
- Progress rendering is covered by a TTY capture check. `security taint-analysis --format json` keeps JSON on stdout and renders phase bars on stderr for rule loading, parsing, source/sink/sanitizer matching, source-reachable callgraph construction, scoped semantic graph construction, source-sink reachability, taint source scheduling, taint chains, finding finalization, flow evidence, and JSON rendering. `--no-progress` / `NO_PROGRESS` disable bars; `--no-color` / `NO_COLOR` now render monochrome progress instead of hiding it.
- A `dvws-node` scale regression was fixed on 2026-06-06. The taint scheduler no longer materializes all semantic function edges before source filtering; it builds per-source source-to-sink IDG cuts and uses only a source-origin resolved-callgraph corridor when concrete sink-node cuts cannot prove completeness. IDG cross-call indexing now caches field-flow call-argument and parameter lookups instead of rescanning segment edges for every field edge. The reported command, `security ../dvws-node taint-analysis --context 512k`, completes with the release binary in 3.44s, with `building source-sink reachability` at 0.191s, `building taint chains` at 0.523s, and 50 JavaScript findings with valid source/sink content.
...
The release CLI is the deployment surface and is unaffected by the local
macOS/Homebrew debug-harness startup behavior. If a future `cargo test` parent
appears idle, inspect child processes before treating it as a Rust harness hang;
some harnesses legitimately wait on child scripts such as
`scripts/validate-mega-cli.py`, and rare local debug binaries can appear at
`_dyld_start` before Rust test code begins. A stalled debug harness is not a
release CLI failure, but it should be sampled and killed rather than left
running.
...
- `crates/workspace/{build.rs,src/{lib,callgraph_sidecar,dataflow,value_flow,flow_ids,taint_index}.rs}`: §D explicit schema/semantic ABIs and exact input fingerprints own cache invalidation; the build fingerprint records release/commit provenance only.
...
This was the investigated inventory of everything standing between the prior state and "perfect on every command and language" - all §A / §C / §D / §E items below are now closed. The mega-flow security regression is GREEN (14/14 + all 20 parser adapters detect their intended synthetic source-to-sink flow). The older §B / §F / §G follow-up labels are superseded by the current readiness section above: Redis, OWASP Benchmark, large-Java performance, and the full workspace `cargo test --workspace --no-fail-fast -- --test-threads=1` green-cert have all passed in later audit runs.
...
5. **§D** cache pipeline-version validation - **DONE** (2026-05-28, redesigned 2026-07-24). Semantic sidecars bind explicit schema/semantic ABIs and their exact compiler inputs. `crates/workspace/build.rs` emits release/commit provenance only and does not force a whole analyzer rebuild after every unrelated source edit.
...
- **§D invalidation ownership corrected (2026-07-24):** `crates/workspace/build.rs` emits `CARGO_PKG_VERSION @ git rev-parse HEAD` as producer provenance and watches the resolved Git HEAD safely across worktrees/submodules. It intentionally does not hash or watch the entire dirty source tree. Structural and flow sidecars reject stale data from their explicit schema/semantic ABI plus exact workspace/dependency, matcher, rulepack/config, and transfer fingerprints. This preserves correctness without relinking the workspace build script and downstream crates after every unrelated edit. `BONSAI_BUILD_FINGERPRINT_OVERRIDE` remains available for reproducible release metadata.
...
3. **§E.3 full-workspace test - RESOLVED (2026-05-29).** `cargo test --workspace --no-fail-fast` run to completion exposed **5704 passed / 40 failed**; prior session's "all gates green" claim was wrong (only the three touched suites had been re-run). All 40 are fixed (§H) across `d4131ff -> e5546be`. A follow-up verification sweep then surfaced one **second-order regression** - `dedup_matrix` (the §H.1/H.2 kit changes made `repo.run()` resolve to two same-named overrides, so source-analysis emitted two rows that render identically) - fixed in `cd63e7c` by keying the source-analysis dedup on the rendered chain. Re-verified: dedup_matrix 15/0, per_lang_cli_matrix 971/0, security_commands 38/0, security --lib 111/0, validate-mega-cli all-pass, and a cross-language source-analysis sweep reports 0 duplicate keys across all 20 mega_flows.
