...
- Rulepack validation is clean: 6,692 rules, 5,485 enabled rules, 9,459 examples, 9,067 enabled examples, 0 errors, 0 warnings.
...
- `cargo test --workspace --no-fail-fast -- --test-threads=1` is green as of 2026-06-06, including doc tests and the 21-language CLI, inspect, adapter, taint, cache, SARIF, and mega-flow suites.
...
- Release CLI command/switch validation passed with `python3 scripts/validate-mega-cli.py --bin ./target/release/bonsai-ninja --skip-realworld` (64 commands for every language except Erlang and Solidity at 63).
...
- Security regression suites passed: `rulepack_conformance` 28/28, `security_pipeline_regressions` 16/16, `branch_merge_audit` all 21, and `sanitizer_fixtures` 6/6.
...
- Progress rendering is covered by a TTY capture check. `security taint-analysis --format json` keeps JSON on stdout and renders phase bars on stderr for rule loading, parsing, source/sink/sanitizer matching, semantic graph construction, source-sink reachability, taint scheduling, taint chains, finding finalization, flow evidence, and JSON rendering. `--no-progress` / `NO_PROGRESS` disable bars; `--no-color` / `NO_COLOR` now render monochrome progress instead of hiding it.
- A `dvws-node` scale regression was fixed on 2026-06-06. The taint scheduler no longer materializes all semantic function edges before source filtering; it builds per-source source-to-sink IDG cuts and only falls back to a source-origin resolved-callgraph corridor when concrete sink-node cuts cannot prove a path. IDG cross-call indexing now caches field-flow call-argument and parameter lookups instead of rescanning segment edges for every field edge. The reported command, `security ../dvws-node taint-analysis --context 512k`, completes with the release binary in 3.44s, with `building source-sink reachability` at 0.191s, `building taint chains` at 0.523s, and 50 JavaScript findings with valid source/sink content.
