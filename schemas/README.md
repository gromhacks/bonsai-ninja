...
- `bonsai-native-export-v9.schema.json` — current Draft 2020-12 schema for
  `bonsai-ninja export --format json` documents whose `schema` is
  `bonsai-native-export` and whose `schema_version` is `9`.
- `bonsai-native-export-v8.schema.json` and
  `bonsai-native-export-v7.schema.json` — immutable historical Draft 2020-12 schemas for
  `bonsai-ninja export --format json` documents whose `schema` is
  `bonsai-native-export` and whose `schema_version` is `8` or `7`.
